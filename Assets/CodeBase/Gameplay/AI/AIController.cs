using UnityEngine;
    ...
    public class AIController : MonoBehaviour
    {
        ...
        /// </summary>
        private float m_RandomSelectMovePointTime;

        /// <summary>
        /// Время между поисками целей. Минимальное внутри реализации 1сек.
        /// </summary>
        private float m_FindNewTargetTime;
        ...
        /// </summary>
        private float m_EvadeRayLength;

        /// <summary>
        /// Кеш ссылка на корабль.
        /// </summary>
        [SerializeField] private EnemyBase m_EnemyPrefab;
        ...
        private void Start()
        {
            m_EnemyPrefab = GetComponent<EnemyBase>();
        ...
        {
            m_EnemyPrefab.TrustControl = m_NavigationLinear;
            m_EnemyPrefab.TorqueControl = ComputeAlignTorqueNormalized(m_MovePosition, transform) * m_NavigationAngular;
        }
    ...
    private enum ActionTimerType
        ...
        private void ActionFire()
        {
            if(m_SelectedTarget != null)
            {
                ...
                {
                    m_EnemyPrefab.Fire(TurretMode.Primary);
        ...
        private Destructible FindNearestDestructibleTarget()
        {
            ...
            {
                if (v.GetComponent<EnemyBase>() == m_EnemyPrefab)
                    continue;

                // исключаем полностью нейтральных (например астероиды)
                if (Destructible.TeamIdNeural == v.TeamId)
                    continue;

                if (m_EnemyPrefab.TeamId == v.TeamId)
                    continue;

                float d2 = (m_EnemyPrefab.transform.position - v.transform.position).sqrMagnitude;
